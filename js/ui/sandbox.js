...
import { VisualIntuitionEngine } from './visual-intuition.js';
...
export class SandboxView {
    static init(containerEl) {
        ...
        containerEl.innerHTML = `
            ...
            <div class="sandbox-panel" id="set-tool">
                ...
                <div id="set-graphics-container" style="margin-top: 1rem;"></div>
            ...
            <div class="sandbox-panel" id="function-tool">
                ...
                <div id="fn-graphics-container" style="margin-top: 1rem;"></div>
            ...
            <div class="sandbox-panel" id="calculus-tool">
                <div class="input-card">
                    <label>Select Calculus Manim Graphic Mode:</label>
                    <select id="calc-mode-select" class="form-select">
                        <option value="derivative">Tangent Line & Slope f'(x) (Manim Graphic)</option>
                        <option value="integral">Riemann Sum Area Accumulation (dx → 0)</option>
                    </select>

                    <button class="btn btn-primary" id="btn-eval-calc" style="margin-top: 1rem;">Render Manim Visualizer</button>
                </div>

                <div id="calc-graphics-container" style="margin-top: 1.5rem;"></div>
    ...
    static bindEvents(containerEl) {
        ...
        this.runFunctionEval(containerEl);
        ...
        if (calcBtn) {
            ...
            this.runCalculusEval(containerEl);
        }
    }
    ...
    static runSetEval(containerEl) {
        ...
        const graphicsContainer = containerEl.querySelector('#set-graphics-container');
        ...
        try {
            ...
            VisualIntuitionEngine.renderVennDiagramSVG(graphicsContainer, setA, setB, op);
    ...
    static runFunctionEval(containerEl) {
        ...
        const graphicsContainer = containerEl.querySelector('#fn-graphics-container');
        ...
        try {
            ...
            VisualIntuitionEngine.renderMappingGraphSVG(graphicsContainer, domain, codomain, mapping);
    ...
    static runCalculusEval(containerEl) {
        const modeSelect = containerEl.querySelector('#calc-mode-select');
        const mode = modeSelect ? modeSelect.value : 'derivative';
        const graphicsContainer = containerEl.querySelector('#calc-graphics-container');

        if (!graphicsContainer) return;

        if (mode === 'integral') {
            VisualIntuitionEngine.renderRiemannArea(graphicsContainer, 0, 3, 8);
        } else {
            VisualIntuitionEngine.renderTangentGraph(graphicsContainer, 'x_squared', 1.5);
        }
    }
}
